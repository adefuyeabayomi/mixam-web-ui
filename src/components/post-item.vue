...
export default {
    ...
    methods: {
        ...
        viewedPost: function () {
            ...
            console.log("user has viewed post", this.viewed)
        },
        view: function () {
            let details = this.postBody.split(" ").slice(0, 9).join(" ");
    ...
    created: function () {
        ...
        this.restoreRepliesThrottingDefault();
        if (this.$store.state.savedContent_Posts[this.pid]) {
            this.itemData = this.$store.state.savedContent_Posts[this.pid];
        } else {
            this.fetchPostData();
        }
        if (this.nearEnd) {
